...
/*        vector<int> vector_mc;
        ...
        // 4.(a) rotation
        poseNormImg =  rotateImage(img, -theta);
        ...
        Point topCenter = Point(stasmVector.at(14).x, 0);
        ...
        Point lipTop = stasmVector.at(62);
        Point lipBottom = stasmVector.at(74);
        Point chinTip = stasmVector.at(6);
        Point bottomCenter = Point(stasmVector.at(6).x, img.rows);
        Point bottomRight = Point(img.cols, img.rows);
        Point topRight = Point(img.cols, 0);
        Point bottomLeft = Point(0, img.rows);
        Point topL = Point(0, 0);

	    cv::imshow("poseNormImg", poseNormImg);
		...
		Mat mask(poseNormImg.rows, poseNormImg.cols, CV_8UC1);

		for(int i=0; i<mask.cols; i++)
			for(int j=0; j<mask.rows; j++)
				if()
				mask.at<uchar>(Point(i,j)) = poseNormImg.at<uchar>(Point(i,j));

		vector<Point> ROI_Poly;

		approxPolyDP(contour, ROI_Poly, 1.0, false);

		fillConvexPoly(mask, &ROI_Poly[0], ROI_Poly.size(), 255, 8, 0);

		Mat imageDest = cvCreateMat(poseNormImg.cols, poseNormImg.rows, CV_8UC3);

		roi.copyTo(poseNormImg, mask);
	    cv::imshow("mask", mask);
